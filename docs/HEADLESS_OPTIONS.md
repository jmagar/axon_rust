So - slight change of plans - instead of mounting my mess of a ~/.claude - I think I'm going to start fresh.

So lets set it up so that the host directory AXON_DATA_DIR/axon/claude mounts to the containers ~/.claude directory.

I do think I want to run with this:
  --strict-mcp-config --mcp-config
    - That way we can have a single source of truth for MCP for Claude
    
Also - we can / should definitely wire up a full on web UI settings page to allow us to configure ALL of these options via the web UI.

`claude mcp` affords us an allow MCP management via the CLI through the WEB UI:
...
`claude agents` allows us to manage agents via the CLI through the WEB UI:
...
# Options we are / should be using

--allow-dangerously-skip-permissions
--chrome
--continue --fork-conversation
...
Can these be wired up to the web UI in some way? It would be handy if we could actually trigger a login flow for Claude Code - I've seen several other apps enable this sort of flow, surely it wasn't as easy as just shelling out to the CLI? Although - that is the UNIX way tho.
...
-claude remote-control	
  - Start a Remote Control session to control Claude Code from Claude.ai or the Claude app while running locally. See Remote Control for flags	
  - `claude remote-control`
...
# Useful Options
    ...
    - `cat file | claude -p "query"`	
        ...
        We could use this to frontload context into the agent for different reasons.
    ...
    --from-pr	
        ...
        Definitely a feature we should implement & start using - need to gather more information about how exactly this works. Is there just one session per PR? How's context work in that session? Etc    

    --append-system-prompt 
        - Append custom text to the end of the default system prompt (works in both interactive and print modes)	
        - `claude --append-system-prompt "Always use TypeScript"`
    
        At some point we should definitely take the time and craft a short system prompt tailored for the Axon project and what it does/wants to achieve
    
    --verbose 
        - Enable verbose logging, shows full turn-by-turn output (helpful for debugging in both print and interactive modes)	
        - `claude --verbose`
    
        Are we planning on using this? I thought we were, can you explain exactly why? I'm not questioning you - just genuiniely curious.
    
    --strict-mcp-config --mcp-config 
        - Only use MCP servers from --mcp-config, ignoring all other MCP configurations	
        - `claude --strict-mcp-config --mcp-config ./mcp.json`
    
        If we setup and used this - then we could have a single config file the user can have for the pulse instance - and they could easily edit it in the editor too and then we wouldnt have to shell out to the CLI and claude mcp add and claude mcp remove etc to manage mcp servers. Plus configuring one of these is the most commonly accepted way to manage mcp servers in the community.
    
    --remote 
        - Create a new web session on claude.ai with the provided task description	
        - `claude --remote "Fix the login bug"`
    
        From our web UI we could have a feature to dispatch cloud agents - that's a handy feature to offer.
    ...
    --no-session-persistence	
        ...
        Allow a user to spin up an ephemeral session
        
    --teleport	
        - Resume a web session in your local terminal
        - `claude --teleport`
        
        Definitely another nice feature to have in our web UI.
        
    --tools
        - Restrict which built-in tools Claude can use (works in both interactive and print modes). Use "" to disable all, "default" for all, or tool names like "Bash,Edit,Read"	
        - `claude --tools "Bash,Edit,Read"`
        
        Granular control is better control.
...
# Full Claude Code CLI Reference  
